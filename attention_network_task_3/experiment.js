...
var test_stimuli = []
var choices = [37, 39]
var path = '/static/experiments/attention_network_task_3/images/'
...
/* set up 4 practice trials. Include all nocue up trials, center cue up trials, double cue down trials, and 6 spatial trials (3 up, 3 down) */
var practice_block = jsPsych.randomization.repeat(test_stimuli.slice(0, 2).concat(test_stimuli.slice(
	18, 19)).concat(test_stimuli.slice(36, 37)), 1, true);
...
for (i = 0; i < block.data.length; i++) {
	...
	attention_network_task_3_experiment.push(first_fixation)

	if (block.data[i].cue == 'nocue') {
		attention_network_task_3_experiment.push(no_cue)
	} else if (block.data[i].cue == 'center') {
		attention_network_task_3_experiment.push(center_cue)
	} else if (block.data[i].cue == 'double') {
		attention_network_task_3_experiment.push(double_cue)
	} else {
		var spatial_cue = {
			type: 'poldrack-single-stim',
		...
		attention_network_task_3_experiment.push(spatial_cue)
	}

	attention_network_task_3_experiment.push(fixation)
	...
	attention_network_task_3_experiment.push(attention_network_task_practice_trial)
	...
	attention_network_task_3_experiment.push(last_fixation)
}

attention_network_task_3_experiment.push(rest_block)
attention_network_task_3_experiment.push(test_intro_block);
...
for (b = 0; b < blocks.length; b++) {
	...
	for (i = 0; i < block.data.length; i++) {
		...
		attention_network_task_3_experiment.push(ANT_trial)
	...
	attention_network_task_3_experiment.push(attention_node)
	attention_network_task_3_experiment.push(rest_block)
}
//attention_network_task_3_experiment.push(post_task_block)
attention_network_task_3_experiment.push(end_block)